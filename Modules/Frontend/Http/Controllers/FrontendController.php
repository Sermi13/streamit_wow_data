...
class FrontendController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Buscar todos os TV Shows e seus episódios
        $tvShows = \Modules\Entertainment\Models\Entertainment::with(['episodes' => function($q) {
            $q->where('status', 1)->orderBy('created_at', 'desc');
        }])->where('type', 'tvshow')->where('status', 1)->get();
    ...
    {
        // Usar a mesma tela de busca, que já exibe lista de TV Shows
        $movieData = collect([]);

        $entertainment_data = \Modules\Entertainment\Models\Entertainment::query()
            ->with([
                'entertainmentGenerMappings',
                'plan',
                'entertainmentTalentMappings',
        ...
        // Buscar TV Shows
        $tvshowList = $entertainment_data->where('type','tvshow')->take(10)->get();
        $tvshowData = (isenablemodule('tvshow') == 1) ? \Modules\Entertainment\Transformers\TvshowResource::collection($tvshowList) : collect([]);
