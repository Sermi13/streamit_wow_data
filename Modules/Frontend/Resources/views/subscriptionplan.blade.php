                ...
                <div class="col">
                    <div class="subscription-plan-wrapper {{ $plan->id == $currentPlanId ? 'active' : '' }} rounded">
                                    ...
                                    <li class="list-desc d-flex align-items-start gap-3 mb-2">
                                        ...
                                        <span class="font-size-16 text-white">
                                            @switch($limitation->limitation_slug)
                                                @case('video-cast')
                                                    {{ $limitation->limitation_value ? 'Acesso às lives transmitidas ao vivo.' : 'Transmissão de vídeo não disponível.' }}
                                                    @break

                                                @case('ads')
                                                    {{ $limitation->limitation_value ? 'Anúncios serão exibidos.' : 'Anúncios não serão exibidos.' }}
                                                    @break

                                                @case('device-limit')
                                                    {{ $limitation->limit == 0 ? 'Você pode usar somente 1 dispositivo móvel.' : "Você pode usar até {$limitation->limit} dispositivo(s) simultaneamente." }}
                                                    @break

                                                @case('download-status')
                                                    Resoluções de download:
                                                    @php
                                                        $availableQualities = [];
                                                ...
                                                @case('supported-device-type')
                                                    ...
                                                    @if (!empty($supportedDevicesList))
                                                        <div class="d-flex align-items-center gap-2 mb-2">
                                                            Compatível com: {{ implode(', ', $supportedDevicesList) }}.
                                                        </div>
                                                    @else
                                                        <div class="d-flex align-items-center gap-2">
                                                            Apenas dispositivo móvel é compatível com este plano.
                                                        </div>
                                                    @endif
                                                @break
                                                ...
                                                @case('profile-limit')
                                                    Você pode criar até {{ $limitation->limit == 0 ? 1 : $limitation->limit }} perfil(is) neste plano para diferentes usuários.
                                                    @break
                        ...
                        <button type="button"
                            ...
                            {{ $plan->id == $currentPlanId ? 'Renovar Plano' : 'Escolher Plano' }}
                        </button>
                    </div>
                </div>
                @endforeach
