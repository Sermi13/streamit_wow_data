...
@section('content')
    ...
    <div class="container-fluid padding-right-0">
        <div class="overflow-hidden">
          ...
          {{-- Lista de TV Shows (igual à página /tv-shows, sem carrossel dinâmico) --}}
          @if(isenablemodule('tvshow')==1)
            <div class="section-wraper mt-5">
                <h2 class="text-white mb-3">TV Shows Populares</h2>
                <div class="row gy-4 row-cols-2 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-6 mt-3">
                    @foreach(\Modules\Entertainment\Models\Entertainment::where('type','tvshow')->where('status',1)->take(12)->get() as $tvshow)
                        <div class="col mb-3">
                            <div class="card border-0">
                                <img src="{{ setBaseUrlWithFileName($tvshow->poster_url ?? $tvshow->poster_tv_url) }}"
                                     alt="{{ $tvshow->name }}"
                                     class="img-fluid rounded">
                                <div class="card-body p-2">
                                    <h6 class="mb-0 text-truncate">{{ $tvshow->name }}</h6>
                                </div>
                            </div>
                        </div>
                    @endforeach
